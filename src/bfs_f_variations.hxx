...
#ifndef __BEST_FIRST_SEARCH_F_VARIATIONS__
#define __BEST_FIRST_SEARCH_F_VARIATIONS__
...
public:
	bool operator()( Node* a, Node* b ) const {
		...
		if( dequal( b->h1n(), a->h1n() ) ){
			if ( dless( a->h2n(), b->h2n() ) ) return true;
...
public:
	...
	BFS_f_VARIATIONS( 	const Search_Model& search_problem ) 
	: m_problem( search_problem ), m_exp_count(0), m_gen_count(0), m_pruned_B_count(0),
	  m_dead_end_count(0), m_open_repl_count(0),m_B( infty ), m_time_budget(infty), m_lgm(NULL), m_max_h2n(0), m_max_h3n(no_such_index), m_verbose( true ) {	
	...
	void			eval( Search_Node* candidate ) {
		...
		if( !candidate->has_state() ){
			...
			m_second_h->eval( *(candidate->parent()->state()), candidate->h2n() );	
	...
	void check_improved_search(Search_Node* candidate){
		if(  candidate->h3n() < m_max_h3n || candidate->h2n() > m_max_h2n){
			m_h3n_improved = true;
	...
	virtual Search_Node*	 	do_search() {
		...
		while(head) {
			...
			// nodes?
			//if( !head->is_helpful() )
			//	std::cout<<"Expanding NON-HELPFUL" << std::endl;
			//if( !head->is_helpful() )
			//			if(head->po_computed())
			{
				eval_po( head );
				check_improved_search( head );
