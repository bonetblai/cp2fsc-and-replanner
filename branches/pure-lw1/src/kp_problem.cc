...
void KP_Instance::calculate_relevant_assumptions(const Plan &plan,
    ...
    for( size_t k = 0; k < plan.size(); ++k ) {
        //current_state.print(cout, *this); cout << endl;
    ...
    //cout << Utils::magenta() << "Backchaining:" << endl;
    int plan_size = plan.size();
    vector<index_set> condition_vec;
    condition_vec.reserve(plan_size);
    index_set open(goal); // signed set of open conditions
    for( int k = plan_size - 1; k >= 0; --k ) {
        const Action &action = *actions_[plan[k]];
        //string mode = k % 2 == 0 ? Utils::red() : Utils::cyan();
        //cout << mode;
        //cout << "open="; write_atom_set(cout, open); cout << endl;
        //cout << "action=" << Utils::yellow() << action.name_ << mode << " is applied at state="; trajectory[k].print(cout, *this); cout << endl;

        // check if action achieves any open condition
        index_set condition, achieved;
        for( index_set::const_iterator it = action.effect_.begin(); it != action.effect_.end(); ++it ) {
            if( (*it > 0) && (open.find(*it) != open.end()) ) {
                condition.insert(action.precondition_.begin(), action.precondition_.end());
                achieved.insert(*it);
            }
        }
        for( size_t i = 0; i < action.when_.size(); ++i ) {
            ...
            for( index_set::const_iterator it = when.effect_.begin(); it != when.effect_.end(); ++it ) {
                if( trajectory[k].satisfy(when.condition_) && (*it > 0) && (open.find(*it) != open.end()) ) {
                    condition.insert(action.precondition_.begin(), action.precondition_.end());
                    condition.insert(when.condition_.begin(), when.condition_.end());
                    achieved.insert(*it);
                }
            }
        }
        for( index_set::iterator it = condition.begin(); it != condition.end(); ) {
            if( *it < 0 ) {
                int literal = *it++;
                condition.erase(literal);
            } else {
                ++it;
            }
        }
        ...
        //cout << "  achieved="; write_atom_set(cout, achieved); cout << endl;
        //cout << "  condition="; write_atom_set(cout, condition); cout << endl;

        // update set of open conditions
        for( index_set::const_iterator it = achieved.begin(); it != achieved.end(); ++it )
            open.erase(*it);
        open.insert(condition.begin(), condition.end());
    }
    //cout << Utils::normal();
    //cout << "open="; write_atom_set(cout, open); cout << endl;
