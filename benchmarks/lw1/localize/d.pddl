(define (domain localize) 
    ...
    (:action move-up
        ...
        :effect (forall (?p ?q - pos) (when (and (at ?p) (up-of ?p ?q) (possible ?q)) (and (at ?q) (not (at ?p)))))
    )
    (:action move-down
        :precondition (not (need-start))
        :effect (forall (?p ?q - pos) (when (and (at ?p) (down-of ?p ?q) (possible ?q)) (and (at ?q) (not (at ?p)))))
    )
    (:action move-left
        :precondition (not (need-start))
        :effect (forall (?p ?q - pos) (when (and (at ?p) (left-of ?p ?q) (possible ?q)) (and (at ?q) (not (at ?p)))))
    )
    (:action move-right
        :precondition (not (need-start))
        :effect (forall (?p ?q - pos) (when (and (at ?p) (right-of ?p ?q) (possible ?q)) (and (at ?q) (not (at ?p)))))
    )
)
