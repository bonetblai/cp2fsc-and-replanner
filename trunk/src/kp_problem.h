...
class KP_Instance : public Instance {
  public:
    ...
    bool is_obs_rule(size_t a) const {
        size_t lower = n_standard_actions_, upper = lower + n_sensor_actions_;
        return (a >= lower) && (a < upper);
    }
    bool is_static_rule(size_t a) const {
        size_t lower = n_standard_actions_ + n_sensor_actions_, upper = lower + n_invariant_actions_;
        return (a >= lower) && (a < upper);
    }
    void apply_plan(const Plan &plan, const State &initial_state, State &final_state, std::vector<State> &support_vec) const;
};
